...
if not has_run and not is_manager:
    st.info("Upload a batch and click **Run agents** to launch the autonomous agents.")
    st.stop()

root_causes = A.get("root_causes", [])
escalations = A.get("escalations", [])

tab_names = ["📊 Overview", "🤖 Agents", "🔍 Root causes", "✅ Approvals"]
